#!/usr/bin/env bash
# Run the frozen scalar step-2000 three-domain H1-H20 discriminator without training.
set -euo pipefail
...
HARD_STOP_UNIT="deepjump-scalar-feedback-h20-hard-stop-$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
shutdown_on_exit() {
  ...
  printf 'Scalar feedback H20 discriminator exit=%s; requesting shutdown at %s\n' \
    "$code" "$(date -Is)"
...
SOURCE_TRAINING_EVIDENCE=${SOURCE_TRAINING_EVIDENCE:?set sealed scalar training_evidence.json path}
H6_DECISION=${H6_DECISION:?set independently read-back H6 decision.json path}
H6_COMPLETION=${H6_COMPLETION:?set independently read-back H6 readback_completion.json path}
...
H6_DECISION_SHA256=ace2b577efadb0c81f4d793ae07708f27c68f100fbd666fba4dabe47ac805e09
H6_COMPLETION_SHA256=90a03a75c53f93291b2c841597080a049b66022f764405193a7fc01b3149c510
...
RUN_DIR="$RUN_ROOT/scalar_feedback_h20_$RUN_ID"
RUNNER_LOG="$RUN_ROOT/scalar_feedback_h20_$RUN_ID.launcher.log"
READBACK_ONE="/tmp/scalar_feedback_h20_readback_one_$RUN_ID"
READBACK_TWO="/tmp/scalar_feedback_h20_readback_two_$RUN_ID"
FINAL_READBACK_ONE="/tmp/scalar_feedback_h20_final_one_$RUN_ID"
FINAL_READBACK_TWO="/tmp/scalar_feedback_h20_final_two_$RUN_ID"
OBS_DST="$BUCKET/deepjump-diagnostics/scalar-feedback-h20/$RUN_ID"
...
[[ -f "$H6_DECISION" && "$(sha256sum "$H6_DECISION" | awk '{print $1}')" == "$H6_DECISION_SHA256" ]] || {
  printf 'H6 decision SHA256 mismatch\n' >&2; exit 2;
}
[[ -f "$H6_COMPLETION" && "$(sha256sum "$H6_COMPLETION" | awk '{print $1}')" == "$H6_COMPLETION_SHA256" ]] || {
  printf 'H6 completion SHA256 mismatch\n' >&2; exit 2;
}
...
printf 'Verified hostname=%s commit=%s clean_tree=true checkpoint=%s H6=%s domains=%s\n' \
  "$(hostname)" "$actual_commit" "$CHECKPOINT_SHA256" "$H6_DECISION_SHA256" \
...
timeout --signal=TERM --kill-after=30s 4m "$PYTHON" -m pytest -q \
  ...
  tests/test_scalar_feedback_h20_adjudication.py \
  tests/test_training_gates.py | tee "$RUN_DIR/pytest.log"

cp "$SOURCE_TRAINING_EVIDENCE" "$RUN_DIR/source_training_evidence.json"
cp "$H6_DECISION" "$RUN_DIR/h6_decision.json"
cp "$H6_COMPLETION" "$RUN_DIR/h6_readback_completion.json"
...
"$PYTHON" -m scripts.adjudicate_scalar_feedback_h20 \
  ...
  --h6-decision "$RUN_DIR/h6_decision.json" \
  --h6-completion "$RUN_DIR/h6_readback_completion.json" \
...
(
  ...
  sha256sum hard_stop_evidence.log obs_prefix_preflight.log pytest.log runtime_evidence.log \
    source_training_evidence.json h6_decision.json h6_readback_completion.json \
...
verify_readback() {
  ...
  command=("$PYTHON" scripts/verify_scalar_feedback_h20_readback.py
...
"$PYTHON" scripts/verify_scalar_feedback_h20_readback.py \
...
"$PYTHON" scripts/verify_scalar_feedback_h20_readback.py \
  ...
  --expected-run-id "$RUN_ID" --expected-deployed-commit "$actual_commit" \
  --expected-obs "$OBS_DST"
printf 'Scalar feedback H1-H20 discriminator complete; training was not started.\n'
