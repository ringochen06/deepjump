...
config=configs/v100_paperstyle_unroll3_lowlr_resume1000.yaml
source_ckpt=runs/v100_paperstyle_unroll3_1000/ckpt_500.pt
ckpt_dir="$repo/runs/v100_paperstyle_unroll3_lowlr_resume1000"
run_dir="$repo/runs/paperstyle_unroll3_lowlr_20260717"

cd "$repo"
mkdir -p "$run_dir/gates"

timeout --signal=TERM --kill-after=2m 30m "$torchrun" --standalone --nproc_per_node=8 \
  scripts/train_ddp.py --config "$config" --resume "$source_ckpt" \
  >"$run_dir/train.log" 2>&1

for step in 750 1000; do
