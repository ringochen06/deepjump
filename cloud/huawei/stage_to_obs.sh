...
# does an intra-region `obsutil sync` (see cloud/huawei/sync_from_obs.sh) which is fast and cheap.
#
...
#   BUCKET=obs://my-mdcath SUBSET=configs/subset_1000.txt bash cloud/huawei/stage_to_obs.sh
#
#   # or stage the FULL dataset (~3.6 TB) instead of a subset
#   BUCKET=obs://my-mdcath MODE=full bash cloud/huawei/stage_to_obs.sh
#
...
# Validate OBS access first:  BUCKET=obs://my-mdcath bash cloud/huawei/obs_roundtrip_test.sh
set -euo pipefail
...
echo "   On the GPU instance:  BUCKET=$BUCKET bash cloud/huawei/sync_from_obs.sh"
