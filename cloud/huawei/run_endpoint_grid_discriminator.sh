#!/usr/bin/env bash
# Run the bounded clean-source 5x5 H1 endpoint discriminator and power off.
set -euo pipefail
...
HARD_STOP_UNIT="deepjump-endpoint-grid-hard-stop-$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
shutdown_on_exit() {
  ...
  printf 'Endpoint grid exit=%s; requesting shutdown at %s\n' "$code" "$(date -Is)"
...
RUN_DIR="$REPO/runs/endpoint_grid_$RUN_ID"
READBACK_DIR="/tmp/endpoint_grid_readback_$RUN_ID"
OBS_DST="$BUCKET/deepjump-diagnostics/endpoint-grid/$RUN_ID"
...
"$PYTHON" -m scripts.adjudicate_endpoint_grid \
...
printf '{"status":"%s","scope":"single_domain_5x5_clean_endpoint_grid","twenty_domain_authorized":false,"second_seed_authorized":false,"confirmation_authorized":false,"formal_training_authorized":false,"run_id":"%s","commit":"%s","checkpoint_sha256":"%s","obs":"%s","completed_at":"%s"}\n' \
...
printf 'Endpoint grid complete with decision=%s; training was not started.\n' "$status"
