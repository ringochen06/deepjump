...
train_pattern='^/data/venvs/deepjump/bin/python /data/venvs/deepjump/bin/torchrun .*v100_paperstyle_unroll3_1000'
run_dir="$repo/runs/paperstyle_unroll3_20260717"
ckpt_dir="$repo/runs/v100_paperstyle_unroll3_1000"
python=/data/venvs/deepjump/bin/python

while pgrep -f "$train_pattern" >/dev/null; do sleep 30; done

for step in 250 500 1000; do
  [[ -s "$ckpt_dir/ckpt_${step}.pt" ]] || {
...
for step in 250 500 1000; do
  ...
  CUDA_VISIBLE_DEVICES=$gpu "$python" "$repo/scripts/robustness_eval.py" \
    ...
    --ode-steps 1 --sample-seed 20260716 --output "$out/fixed_ode1.json" \
  ...
  CUDA_VISIBLE_DEVICES=$gpu "$python" "$repo/scripts/rollout_robustness_eval.py" \
    --ckpt "$ckpt_dir/ckpt_${step}.pt" --domains 5 --starts 5 --steps 20 \
    --methods ode_1 --seed 20260716 --output "$out/rollout20_ode1.json" \
...
status=0; : >"$run_dir/gates/status.tsv"
for i in "${!pids[@]}"; do
