#!/usr/bin/env bash
# Run the bounded same-domain full-tensor learnability diagnostic and always power off.
set -euo pipefail
...
CONFIG=configs/v100_tensorcloud01_full_d1_tiny_domain5000.yaml
DOMAIN_LIST=configs/tiny_overfit_domain_1a0hA01.txt
DOMAIN_LIST_SHA256=${DOMAIN_LIST_SHA256:-3da0d5c44e5d1a68aa5e99d01acf296725f7e8f94ca990547991b0d87bcf0d9d}
TRAIN_DIR="$REPO/runs/v100_tensorcloud01_full_d1_tiny_domain5000"
RUN_DIR="$REPO/runs/full_tensor_tiny_domain_overfit5000_$RUN_ID"
READBACK_DIR="/tmp/full_tensor_tiny_domain_overfit5000_readback_$RUN_ID"
OBS_DST="$BUCKET/deepjump-diagnostics/full-tensor-tiny-domain-overfit5000/$RUN_ID"
...
mkdir -p "$RUN_DIR"
exec > >(tee -a "$RUN_DIR/diagnostic.log") 2>&1
...
[[ "$(sha256sum "$DOMAIN_LIST" | awk '{print $1}')" == "$DOMAIN_LIST_SHA256" ]] || {
  printf 'one-domain list SHA256 mismatch\n' >&2
  exit 2
}
...
"$PYTHON" -m pytest -q \
  ...
  tests/test_tiny_domain_overfit_adjudication.py \
...
for step in 4000 4500 5000; do
  "$PYTHON" scripts/validate_training_checkpoint.py \
    --checkpoint "$TRAIN_DIR/ckpt_${step}.pt" \
...
"$PYTHON" scripts/adjudicate_tiny_domain_overfit.py \
  --history "$TRAIN_DIR/history.json" \
  --domain-list-sha256 "$DOMAIN_LIST_SHA256" \
  --selection-only --output "$RUN_DIR/selection.json"
selected_step=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["selected_step"])' "$RUN_DIR/selection.json")
SELECTED_CHECKPOINT="$TRAIN_DIR/ckpt_${selected_step}.pt"
[[ -f "$SELECTED_CHECKPOINT" ]] || { printf 'selected checkpoint was not retained\n' >&2; exit 2; }

CUDA_VISIBLE_DEVICES=0 timeout --signal=TERM --kill-after=30s 10m \
  "$PYTHON" scripts/rollout_robustness_eval.py \
  --ckpt "$SELECTED_CHECKPOINT" \
  --domain-list "$DOMAIN_LIST" --domain-list-sha256 "$DOMAIN_LIST_SHA256" \
  --domains 1 --starts 5 --steps 20 --methods mean,ode_1 \
  --seed 20260718 --integrator euler --tau-max 1.0 --drift-anchor state \
  --output "$RUN_DIR/rollout.json" \
  > "$RUN_DIR/rollout.log" 2>&1

"$PYTHON" scripts/adjudicate_tiny_domain_overfit.py \
  --history "$TRAIN_DIR/history.json" --rollout "$RUN_DIR/rollout.json" \
  --domain-list-sha256 "$DOMAIN_LIST_SHA256" \
...
"$PYTHON" scripts/validate_training_checkpoint.py \
  --checkpoint "$READBACK_DIR/training/ckpt_${selected_step}.pt" \
  --history "$READBACK_DIR/training/history.json" \
  --expected-step "$selected_step" --expected-world-size 8 --expected-delta 1 \
  --require-full-tensor --history-mode contains \
  --output "$RUN_DIR/obs_selected_gate.json"

decision_status=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["status"])' "$RUN_DIR/decision.json")
selected_sha=$(sha256sum "$SELECTED_CHECKPOINT" | awk '{print $1}')
printf '{"status":"%s","scope":"same_domain_mechanism_diagnostic_only","formal_training_authorized":false,"run_id":"%s","commit":"%s","selected_step":%s,"selected_checkpoint_sha256":"%s","obs":"%s","completed_at":"%s"}\n' \
  "$decision_status" "$RUN_ID" "$actual_commit" "$selected_step" "$selected_sha" "$OBS_DST" "$(date -Is)" \
  | tee "$RUN_DIR/summary.json"
(
  cd "$RUN_DIR"
  sha256sum decision.json selection.json rollout.json data_audit.json \
    local_ckpt_*_gate.json obs_selected_gate.json summary.json
...
printf 'Tiny-domain diagnostic complete with decision=%s; formal training was not started.\n' "$decision_status"
